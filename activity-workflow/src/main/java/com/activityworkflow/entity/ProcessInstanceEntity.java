...
/**
 * One row per state transition for a given {@code businessId}.
 *
 ...
 * created_time for that businessId/businessService.
 */
@Entity
@Table(name = "aw_process_instance",
       indexes = {
           @Index(name = "idx_pi_business_id",      columnList = "business_id"),
           @Index(name = "idx_pi_business_service", columnList = "business_service"),
           @Index(name = "idx_pi_lookup",           columnList = "business_service,business_id,created_time")
       })
@Data
@Builder
@NoArgsConstructor
