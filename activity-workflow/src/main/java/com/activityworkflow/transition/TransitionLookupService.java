...
public class TransitionLookupService {
    ...
    public List<ProcessStateAndAction> getProcessStateAndActions(
        ...
        Map<String, ProcessInstanceEntity> latestByBusinessId =
                fetchLatestByBusinessId(processInstances, workflow.getActivityName());
        ...
        for (ProcessInstanceDTO dto : processInstances) {
            ProcessStateAndAction tuple = ProcessStateAndAction.builder()
                    ...
                    .processInstanceFromDb(latestByBusinessId.get(dto.getBusinessId()))
                    .build();
                ...
                currentState = workflow.getStates().stream()
                        ...
                        .orElseThrow(() -> new InvalidTransitionException(
                                "Workflow '" + workflow.getActivityName()
                                        + "' has no isStartState=true and businessId '"
                                        + dto.getBusinessId() + "' has no existing transitions"));
            }
            ...
            if (isTransitionCall) {
                if (tuple.getAction() == null) {
                    throw new InvalidTransitionException(String.format(
                            "INVALID ACTION: Action '%s' not found in config for businessId '%s' (current state: '%s')",
                            dto.getAction(), dto.getBusinessId(), currentState.getStateName()));
                }
    ...
    private Map<String, ProcessInstanceEntity> fetchLatestByBusinessId(
        ...
        for (ProcessInstanceDTO dto : processInstances) {
            Optional<ProcessInstanceEntity> latest =
                    processRepository.findLatest(businessService, dto.getBusinessId());
            latest.ifPresent(p -> map.put(dto.getBusinessId(), p));
        }
        return map;
    }
